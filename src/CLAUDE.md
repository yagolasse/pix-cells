...
| `selection.cpp` | Free functions `lift_selection(CanvasState&, SelectionState&)` and `commit_floating(CanvasState&, SelectionState&)` — floating-selection helpers declared in `app_state.h` |
| `cursor_manager.cpp` | Logs SDL_CreateColorCursor failures to help diagnose invisible cursors |
