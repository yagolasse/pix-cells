...
| `ui_scale.h/cpp` | `ui_scale::init(fonts[5], base_style)`, `::apply(scale)` (accepts 0.75/1.0/1.25/1.5/2.0, clamps to nearest), `::get()` → current scale, `::px(base)` → base × scale, `::save()`/`::load()` to persist/restore from `SDL_GetPrefPath/settings.ini` |
| `input_handler.h/cpp` | `input_handler::handle_keyboard(AppState&)` — centralizes all keyboard input handling (tool shortcuts, undo/redo, copy/cut/paste/delete, selection commands). Called once per frame in `main.cpp` when no modal dialog is open. |
| `main.cpp` | SDL3+ImGui init, main loop; delegates keyboard handling to `input_handler::handle_keyboard(app)` |
| `workbench.h/cpp` | Fullscreen dockspace (`BeginWorkbench`) — calls `BuildDefaultLayout` automatically on first launch (when no ini node exists) before registering the DockSpace; docks 6 panels: Tools, Tool Settings, Canvas, Color, Layers, Timeline; Log floats freely when toggled |
