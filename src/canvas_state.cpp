...
static uint32_t blend_over(uint32_t dst, uint32_t src) {
    ...
    return ((uint32_t)(oa * 255.0f) << 24) | ((uint32_t)(ob * 255.0f) << 16) | ((uint32_t)(og * 255.0f) << 8) |
           (uint32_t)(or_ * 255.0f);
}
...
int CanvasState::width() const {
    return layers.empty() ? 0 : layers[0].canvas.width;
}
int CanvasState::height() const {
    return layers.empty() ? 0 : layers[0].canvas.height;
}
Canvas& CanvasState::active() {
    return layers[active_layer].canvas;
}
