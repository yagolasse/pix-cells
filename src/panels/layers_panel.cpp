...
void panels::SetLayersIconFont(ImFont* font) { s_icon_font = font; }
...
static ImVec4 layer_thumb(const Layer& layer) {
    ...
    for (auto& p : pts) {
        if (!c.in_bounds(p[0], p[1])) continue;
        uint32_t px = c.get(p[0], p[1]);
        if (((px >> 24) & 0xFF) > 128)
            return { (px & 0xFF) / 255.f, ((px >> 8) & 0xFF) / 255.f,
                     ((px >> 16) & 0xFF) / 255.f, 1.f };
    }
...
void panels::DrawLayers(CanvasState& cs) {
    ...
    ImGui::Text("Layers \xc2\xb7 %d", (int)cs.layers.size());
    ...
    if (ImGui::Button(ICON_FA_PLUS "##ladd", {btn_sz, btn_sz})) {
        ...
        cs.active_layer++;
        Log("Layer added: \"%s\"", cs.layers[cs.active_layer].name.c_str());
    ...
    if (ImGui::Button(ICON_FA_COPY "##ldup", {btn_sz, btn_sz})) {
        cs.push_snapshot();
        Layer copy = cs.layers[cs.active_layer];
        copy.name += " copy";
        cs.layers.insert(cs.layers.begin() + cs.active_layer + 1, std::move(copy));
        cs.active_layer++;
        Log("Layer duplicated: \"%s\"", cs.layers[cs.active_layer].name.c_str());
        cs.rebuild_composite();
    }
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Duplicate layer");

    ImGui::SameLine(0, spacing);
    ImGui::BeginDisabled((int)cs.layers.size() <= 1);
    ...
    if (cs.active_layer >= 0 && cs.active_layer < (int)cs.layers.size()) {
        auto& layer = cs.layers[cs.active_layer];
        ...
        ImGui::SetNextItemWidth(-1);
        ImGui::Combo("##blend", &layer.blend_mode, blend_modes, IM_ARRAYSIZE(blend_modes));
    }
