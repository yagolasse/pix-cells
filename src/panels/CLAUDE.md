...
- **Shape preview**: while `shape_dragging`, the preview overlay uses the same pixel-exact algorithms as the commit path (inline Bresenham + stamp for line, row/col loops for rect, dual-scan for ellipse) — each canvas pixel renders as an `AddRectFilled` quad of size `cs.zoom`. Tool 9 (select) keeps a vector marching-ants overlay instead.
